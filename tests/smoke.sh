...
codex_output="$(PATH="$FAKE_BIN:$PATH" "$CLI" codex --account personal --workspace "$WORKSPACE" -- exec --profile deep 'review this repo')"
printf '%s\n' "$codex_output" | grep -q "HOME=$WORKSPACE/.codex-home/profiles/personal"
printf '%s\n' "$codex_output" | grep -q 'ARGS=-C'
printf '%s\n' "$codex_output" | grep -q 'exec --profile deep review this repo'
set +e
